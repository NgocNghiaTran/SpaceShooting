...
public class SpanwerEnemy : MonoBehaviour {
    ...
    [SerializeField] private GameObject[] _upgradeBullets;  // Mảng các loại UpgradeBullet
    [SerializeField] private float upgradeBulletSpawnChance = 0.1f;
    ...
    IEnumerator Spanwer(){
        ...
        // Sinh UpgradeBullet với tỉ lệ thấp
        if (Random.value < upgradeBulletSpawnChance)
        {
            SpawnUpgradeBullet();
        }
    ...
    // Hàm sinh UpgradeBullet
    void SpawnUpgradeBullet()
    {
        float minX = -_box.bounds.size.x / 2;
        float maxX = _box.bounds.size.x / 2;
        Vector3 temp = transform.position;
        ...
        // Chọn một UpgradeBullet ngẫu nhiên và sinh nó
        int upgradeBulletIndex = Random.Range(0, _upgradeBullets.Length);
        Instantiate(_upgradeBullets[upgradeBulletIndex], temp, Quaternion.identity);
    }
