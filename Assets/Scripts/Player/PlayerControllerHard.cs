...
public class PlayerControllerHard : MonoBehaviour
{
    ...
    void OnTriggerEnter2D(Collider2D target)
    {
        ...
        if(target.tag == "UpgradeBullet")
        {
            Destroy(target.gameObject);
        }

        if (target.tag == "Security")
        {

            StartCoroutine(TriggerSecuri());
        }
    }

    IEnumerator TriggerSecuri()
    {
        securi.SetActive(true);
        yield return new WaitForSeconds(6);
        securi.SetActive(false);
